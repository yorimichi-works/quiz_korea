...
const state = {
  ...
  matchId: null, phaseDeadline: null, answerRightLost: false,
...
function matching({ resume = false } = {}) {
  ...
  if (!resume) {
    ...
    state.phaseDeadline = Date.now() + 1300;
  }
...
function battleReady() {
  state.phase = 'ready'; state.phaseDeadline = null; persistSession();
...
function countdown({ resume = false } = {}) {
  ...
  if (!resume || !state.phaseDeadline) state.phaseDeadline = Date.now() + 2550;
  persistSession();
...
function battle({ resume = false } = {}) {
  ...
  if (!resume) {
    state.phase = 'reading'; state.phaseDeadline = Date.now() + 6800; state.answerRightLost = false;
    state.selectedChars = []; state.charIndex = 0; state.answerRemaining = state.answerSeconds;
  }
  ...
  const tick = () => {
    ...
    if (fill) fill.style.width = `${Math.min(100, remaining / (locked ? 70 : 68))}%`;
    if (!locked && remaining <= 5000) document.querySelector('#status').textContent = '문제 전체가 공개되었습니다 · 제한 시간 안에 눌러주세요';
...
function claimAnswer() {
  ...
  state.phaseDeadline = Date.now() + state.answerSeconds * 1000; state.answerRightLost = false; clearTimer(); persistSession();
...
function judge(correct) {
  if (state.phase !== 'answering') return; clearTimer(); state.phase = 'result'; state.phaseDeadline = Date.now() + 2000;
...
function matchResult() {
  clearSavedSession(); state.phase = 'match-result'; state.phaseDeadline = null; const won = state.score >= 5 || state.lives > 0;
